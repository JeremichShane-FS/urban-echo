...
const getAboutContent = async () => {
  try {
    console.log("🚀 Starting about content fetch...");

    const data = await strapiRequest("about-contents?populate=*");
    console.log("✅ Strapi request successful:", data);

    const content = data.data?.[0];
    console.log("📄 Content extracted:", content);

    if (!content) {
      console.log("❌ Throwing 'No content found' error");
      throw new Error("No content found");
    }

    console.log("🎯 About to return successful result");
    return {
    ...
    console.error("❌ About content error details:", error);
